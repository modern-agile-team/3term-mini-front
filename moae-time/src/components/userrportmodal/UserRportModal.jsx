...
function UserRportModal (props) {
  ...
  const handleClickOutside = ({ target }) => {
    if (userReport && !modalEl.current.contains(target)) {
      setUserReport(false);
    }
  };
  ...
  return (
    <>
      {userReport && (
        <ModalBackGround modalState={userReport}>
          <Modal
            ref={modalEl}
            modalState={userReport}
            width='400px'
            height='480px'
            topMargin='121px'
            leftMargin='556px'
